...
class Aug_Encoder(torch.nn.Module):
    def __init__(self, input_dim, encoder, augmentor, args):
    ...
    def forward(self, x, edge_index, batch):
        ...
        #mask = torch.sigmoid(torch.transpose(self.mlp_edge_model(torch.transpose(self.mask,0,1)),0,1)[:edge_index.shape[1], :])
        edge_index_new = edge_index[:, mask[:, 0].bool()]
        if(x != None):
            aug1, aug2 = self.augmentor
            #x1, edge_index1, edge_weight1 = aug1(x, edge_index)
            ...
            z2, g2 = self.encoder(x, edge_index_new, batch)
            return mask, g1, g2
        else:
