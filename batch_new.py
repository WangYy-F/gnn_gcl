...
def val_epoch(aug_encoder_model, encoder_model, contrast_model, dataloader, epoch, args):
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(dataloader):
            ...
            new_adj_matrix, _, _ = aug_encoder_model(data.x, data.edge_index, data.batch)
            edge_mask = torch.round(new_adj_matrix).cpu().detach().to(args.device).bool()
            new_edge_index = data.edge_index[:, edge_mask[:, 0]].int()
...
def train_epoch(aug_encoder_model, encoder_model, contrast_model, dataloader, aug_optimizer, optimizer, epoch, args):
    ...
    for batch_idx, data in enumerate(dataloader):
        ...
        new_adj_matrix, g1, g2 = aug_encoder_model(data.x, data.edge_index,data.batch)
        edge_mask = torch.round(new_adj_matrix).cpu().detach().to(args.device).bool()
        new_edge_index = data.edge_index[:, edge_mask[:, 0]].int()
        ...
        if (data.x != None):
            ...
            if args.contrast_mode == 'G2G':
                ...
                g1, g2, g2_pos = [encoder_model.encoder.project(g_) for g_ in [g1, g2, g2_pos]]
                # g1.shape: [batch_size, num_embeddings]
                l2 = contrast_model(g1=g1, g2=g2, g2_pos=g2_pos, batch=data.batch)
            loss = l1 + l2
            print(loss)
        else:
            loss = l1
...
#log
args.add_argument('--log_dir', default='experiments_new_batch_1', type=str)
...
mini_loader = DataLoader(dataset, batch_size=args.batch_size)
