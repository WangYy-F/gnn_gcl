...
def drop_feature(x: torch.Tensor, drop_prob: float):
    device = x.device
...
def drop_edge_by_weight(edge_index, weights, drop_prob: float, threshold: float = 0.7):
    ...
    print(len(drop_mask[0]),edge_index[:, drop_mask[0]].size())
    return edge_index[:, drop_mask[0]]
...
def add_edge(edge_index: torch.Tensor, ratio: float):    # return: -> torch.Tensor:
    num_edges = edge_index.size()[1]
    num_nodes = edge_index.max().item() + 1
