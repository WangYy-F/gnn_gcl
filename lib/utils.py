...
def normalize_batch_embedding(graph_embedding):
    feature_shape = graph_embedding.size()
    batch_mins, _ = torch.min(graph_embedding, dim=-1, keepdim=True)
    batch_maxs, _ = torch.max(graph_embedding, dim=-1, keepdim=True)
    normalized_batch_embedding = torch.div(graph_embedding - batch_mins,
